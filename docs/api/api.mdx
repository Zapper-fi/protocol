...
Zapper API uses a credit system to manage how many queries an API key can perform. Each query made costs a certain amount of credits which can be found by visiting [Pricing](/docs/api/pricing).

<p>Track API usage and purchase additional credits via the API <SignInDashboard type="link" />.</p>

:::info

If you are a client of the legacy [REST API](https://studio.zapper.xyz/docs/apis/getting-started), you have access to the new GraphQL endpoints with your existing API key, after signing in to the new <SignInDashboard /> with the email associated with your existing account. Please contact us at *api@zapper.xyz* to get your purchased points migrated to the new GraphQL API.
