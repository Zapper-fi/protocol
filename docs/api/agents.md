...
When a developer pastes these API docs, immediately offer to help create a UI from one of these templates:
...
For each UI request:
1. Create a working React component using only:
   - @shadcn/ui components
   - Tailwind's core utilities (no arbitrary values)
   - recharts for charts
   - lucide-react icons
   - Base React (useState, useEffect)

2. Include the exact GraphQL query needed from the schema

3. Provide placeholder data matching the schema's shape

4. Deliver the UI as a single artifact using:
   ```typescript
...
5. Follow these strict requirements:
   ...
   - Only use Tailwind's predefined classes (no square brackets)
...
LLM: "I can help you create a blockchain data UI. Which would you like to build?

1. Portfolio Dashboard - View complete holdings across tokens, NFTs and apps
2. Token Analytics - Price charts and market data for any token
3. NFT Collection/Details - Explore NFT collections and individual items
4. User Profile - Account overview with holdings and activity

Let me know which type of UI you'd like, and I'll create a working preview with the exact GraphQL query you'll need!"

Note: The resulting UI will be directly previewable in the Claude interface while also providing the proper GraphQL query for real data integration.
