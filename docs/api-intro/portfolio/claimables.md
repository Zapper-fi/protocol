...
Let's say you want to show a user all the tokens they can claim across all the onchain apps they have used and how much it's worth in USD. You would pass `address` for the user and return the `claimables` object, with fields such as `address`, `balanceUSD`, `balance`, `symbol`, `price`, and `network`. Example of the response below:
